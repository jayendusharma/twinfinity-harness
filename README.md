...
Changes that share SQLite transitions or supervisor behavior may be developed separately, but they are installed only from a validated integration head. Live endpoint installation, service activation, and database replacement remain explicit Planner-controlled operations; a pushed branch is not an installation or deployment. The reviewed source-catalog target is Planner v3 / Development v6 / SRE v6. Planner v2 and Development/SRE v3 remain their exact direct rollback endpoints, v4 is retained historical hardening, and v5 alone remains dormant broker-only readiness hardening. Planner v3 stays read-only except for the one-use `planner-park/v1` boundary for a strictly typed claimed-no-delivery PARK notice; source presence is not installation or activation evidence.
