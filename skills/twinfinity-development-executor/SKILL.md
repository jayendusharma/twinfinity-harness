...
6. Publish only with the canonical installed `prepush_control.py guarded-push` after its exact-head PASS receipt. Invoke it directly through `/usr/bin/python3`, with no shell wrapper or leading environment assignment, and pass the active owning issue even when a reviewed transfer retains an older surface branch. Raw `git push`, hook bypass, alternate publication, no-op commits, and unchanged-head CI reruns are prohibited.
