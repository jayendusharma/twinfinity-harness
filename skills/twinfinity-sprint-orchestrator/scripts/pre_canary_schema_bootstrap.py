...
Supported only while the stopped owner is the sole database and namespace
mutator from opening through connection close. External mutation (including
accidental or uncooperative same-UID replacement) is unsupported. Identity
observations and advisory locks do not enforce that exclusion. A pre-COMMIT
failure rolls back logical transaction effects, not necessarily file bytes or
transient sidecars. Successful COMMIT remains APPLIED despite cleanup errors.
"""
...
PREDECESSOR_SCHEMA_SENTINEL_SHA256 = (
    "e33a79655ccc0cd2b810c34c4683f2ad119720683a7c169735410097a854d4c5"
)
RESULT_SCHEMA_SENTINEL_SHA256 = (
    "8fdd39d2fb7aa8ec70d4809c73cc29e939b4daf0ae4cb422fc44c6d2dd2ee7c9"
)
...
def _expected_predecessor_tables() -> list[dict[str, str]]:
    objects = _canonical_predecessor_objects()
    return [
        {"table": table, "schema_sha256": _bound_schema_digest(table, objects)}
...
def _result_sentinel() -> dict[str, Any]:
    objects = _canonical_predecessor_objects() + _canonical_missing_objects()
    return {
...
def _objects_owned_by_missing_tables(
    ...
    return _objects_owned_by_tables(connection, MISSING_TABLES)


def _objects_owned_by_tables(
...
def _predecessor_object_tables() -> tuple[str, ...]:
    # approval_events INSERT also updates SQLite's AUTOINCREMENT counter.
    # Bind every accepted predecessor family, including all possible targets
    # of trigger DML, and the system-owned sqlite_sequence declaration.
    return tuple(sorted(set(EXPECTED_SCHEMA_SHA256) - set(MISSING_TABLES))) + (
        "sqlite_sequence",
    )


def _canonical_predecessor_objects() -> list[dict[str, str]]:
    """Compile accepted definitions in memory, never initialize the target DB."""
    ...
    try:
        ...
        control = HostedOperationControl.__new__(HostedOperationControl)
        control.store = store
        ...
        if present != set(EXPECTED_SCHEMA_SHA256):
            raise BootstrapHold("PRE_CANARY_BOOTSTRAP_SOURCE_SENTINEL_DRIFT")
        return _objects_owned_by_tables(connection, _predecessor_object_tables())
    finally:
        connection.close()


def _bound_schema_digest(
    table: str, objects: Sequence[Mapping[str, str]]
) -> str:
    # Preserve the sentinel wire shape: its existing per-table schema digest
    # now includes the complete SQL object family as well as columns/FKs.
    family = [
        item for item in objects
        if item["table"] == table
        or (table == "approval_events" and item["table"] == "sqlite_sequence")
    ]
    family.sort(key=lambda item: (item["type"], item["name"]))
    return digest_json({
        "columns_and_foreign_keys_sha256": EXPECTED_SCHEMA_SHA256[table],
        "objects": _schema_object_manifest(family),
    })
...
def _accepted_schema_state(
    ...
    actual_objects = _objects_owned_by_tables(connection, _predecessor_object_tables())
    if _schema_object_manifest(actual_objects) != _schema_object_manifest(
        _canonical_predecessor_objects()
    ):
        raise BootstrapHold("PRE_CANARY_BOOTSTRAP_PREDECESSOR_OBJECT_DRIFT")
...
def apply_schema_bootstrap(
    ...
    *,
    ...
    """Apply under sole stopped ownership, or return the exact durable replay.

    No observation can close the gap to a later COMMIT against an external
    namespace mutator. Such interference violates the supported boundary and
    provides no safety, replay, or automatic-retry guarantee.
    """
    ...
    try:
        ...
        if state == "RESULT":
            ...
            durable_receipt = replay_receipt
        ...
        if durable_receipt is None and connection is not None and connection.in_transaction:
            connection.execute("ROLLBACK")
        raise
    finally:
        ...
        if anchor is not None:
            try:
                ...
                cleanup_failure = exc
        # Both cleanup attempts run. Neither can turn a known durable result
        # into HOLD, imply rollback after COMMIT, or change the receipt schema.
        if cleanup_failure is not None and durable_receipt is None:
            raise BootstrapHold("PRE_CANARY_BOOTSTRAP_CLEANUP_FAILED") from cleanup_failure
