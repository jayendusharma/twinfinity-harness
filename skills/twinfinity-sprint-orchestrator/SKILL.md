...
If an ordinary Development or SRE admission remains `PREPARED` and unclaimed through its third unchanged transport attempt, the supervisor atomically holds the admission message, wake, bound watch, and retained item and emits exactly one current-Planner notice. Only the current Planner may consume that exact notice and run `scripts/kanban_pull_buffer.py recover-unclaimed-admission --transaction-file <json>`; the fenced transaction retires the admitted candidate once, advances generation, restores `PREPARED/NONE`, clears owner and lease, preserves demand, marks prior readiness `STALE`, and emits one refill event. Claim evidence, active or terminal delivery lineage, stale fences, or unauthorized callers remain write-free `HOLD` conditions.
