...
6. For a ready-eligible candidate, close mechanical Planner guards in one continuation: reconcile the volatile GitHub projection, publish `agent-ready` only as a READY effect, verify global endpoint transport, refresh all bindings, register the immutable READY packet, and select the highest-value safe outcome. Capacity is a ceiling, not a utilization target; leave it open when no valuable collision-free work is ready.
