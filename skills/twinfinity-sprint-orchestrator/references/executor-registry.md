...
Persist only the token digest. Runtime-selectable direct endpoints—Development source-current v6, its explicit v3 rollback, Planner v2, and SRE v3—pass the raw token only in the child environment. Historical v4 and broker-only v5 are not runtime-selectable. Brokered v5 readiness keeps the token exclusively in the outer owner process and uses a fixed non-authorizing child prompt.
