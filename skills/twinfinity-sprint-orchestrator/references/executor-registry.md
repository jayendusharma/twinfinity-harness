...
Each reservation records an authoritative target-progress digest and each terminal transition reads it back. Exit zero with an unchanged digest is `EXECUTOR_TARGET_NO_PROGRESS`, not success. Supervisor wake retries retain a three-attempt budget for identical phase, payload, and progress; the budget resets only after authoritative progress changes, and exhaustion moves the exact message or terminal watch to a typed durable `HOLD`.
