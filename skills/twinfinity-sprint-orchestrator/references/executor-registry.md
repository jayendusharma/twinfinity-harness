...
| --- | --- | --- | --- |
| Planner | `role.planner.v3` | `171f222f746e99c16182e38957eefb012576720c7d0dd8ebe01e2096bec45e1b` | source-current read-only `planner-park/v1` cutover target; not proof of installation or activation |
| Planner | `role.planner.v2` | `38d39166c7573d676206a0f70efd4ebbc68c2d74cd743bab85f48de56b5128cf` | exact direct rollback endpoint |
...
Validate the catalog from source or a staged atom with `executor_registry.py --profile-root <profile-directory> audit-config`. Do not install or activate any endpoint merely because its source artifacts validate. Planner v3 and execution-role v6 installation and compare-and-swap activation remain separate stopped-state operations; the source catalog preserves Planner v2 and the corresponding execution-role v3 endpoints for exact rollback. Any future v5 work requires a separate reviewed source change, complete missing mechanics, explicit installation authority, and a fresh pointer decision.

### Planner v3 PARK boundary

`planner-park/v1` does not make a Planner notice mutating. The ordinary path remains read-only. For the exact closed claimed-no-delivery schema only, the owner runner replaces the prompt argument with stdin, removes the raw token and database path from the child, and arms a private one-use capability after the final zero-WAL heartbeat barrier. The immutable manifest binds the exact installed Codex 0.147 binary and version, Python interpreter, runner, controller, guard, recursively resolved local imports, profile, effective config, managed requirements, argv, cwd, process start identities, and cgroups.

The source registry encodes this protocol through the exact `role.planner.v3` / version 3 identity rather than a new raw TOML key, so the accepted-main registry auditor can still validate the candidate profile catalog. The current loader derives the immutable `planner-park/v1` execution protocol from that identity and rejects an injected Planner `execution_protocol` field.

The only allowed nested tool is the fixed controller command through actual synchronous Bash `PreToolUse`. The hook binds its raw event bytes plus session, turn, tool-use, model, permission mode, cwd, and process lineage. The distinct controller must consume and adopt through the same socket before it receives the credential in its own process. Every specialized tool, changed command, wrong process, duplicate or malformed event, byte drift, timeout, crash, or absent hook fails closed. The runner suppresses heartbeats from prompt release through adoption, keeps the SQLite writer reservation until adoption, kills and proves quiescence of the complete Codex process group, and records typed terminal state. Official replay completes a fresh matching PARK message without a second Git/GitHub observation or a second capacity release.
...
Persist only the token digest. Runtime-selectable direct endpoints—Development/SRE source-current and explicit v3 rollback, plus Planner v2 rollback—pass the raw token only in the child environment. Planner v3 instead keeps it in the owner runner until the reauthenticated controller consumes the one-use capability. Historical v4 and broker-only v5 are not runtime-selectable. Brokered v5 readiness keeps the token exclusively in the outer owner process and uses a fixed non-authorizing child prompt.
...
The source-current Planner endpoint loads its exact versioned `twinfinity-planner-v3` profile, must read `twinfinity-sprint-orchestrator/SKILL.md`, remains non-coding, and has no sandbox write root. Only the exact PARK controller described above may cross that read-only boundary. Planner v2 remains the direct rollback profile and does not inherit the v3 protocol. A direct Development or SRE endpoint likewise loads the exact versioned profile bound to its immutable manifest and reads its role skill before acting. A future brokered v5 evaluator receives only the v5 profile, its self-contained digest-bound readiness instruction, the exact receipt schema, and canonical broker inputs inside its private mount tree; it does not receive the mutable generic Development or SRE skill tree.
