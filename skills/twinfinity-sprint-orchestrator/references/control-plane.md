...
`prepare-terminal-closeout` accepts exactly one `twinfinity-terminal-closeout-packet/v1`. It revalidates the current role endpoint and `RUNNING` target-bound attempt, original admission and claim, active watch, item/source/generation/lease, typed `twinfinity-terminal-receipt/v1`, typed `twinfinity-terminal-cleanup/v1`, and the deterministic owning-issue publication. In one transaction it records the immutable packet, enqueues exactly one idempotent comment outbox row, completes the locally consumed admission, and moves the item to persisted `PUBLICATION_PENDING`. Allocation remains `ACTIVE`; role/shared units, lease, accountable endpoint, and watch remain held. Exact replay returns the same packet and outbox without republishing.

The outbox must reach `COMPLETE` with exact `comment:<id>` remote readback. Until then the derived terminal state is `PUBLICATION_PENDING` or `PUBLICATION_HOLD`, never `DONE`. Once readback is exact, `commit-terminal-closeout` revalidates the immutable packet, publication body and digest, role endpoint, attempt, admission and claim, watch, item, receipt, cleanup, lease, and absence of pending pre-push or artifact-GC work. It then commits one SQLite transaction containing `DONE/NONE`, terminal-watch `COMPLETE`, lease/allocation/capacity release, one immutable terminal commit, and one refill dirty event. A fresh exact same-lineage terminal-watch attempt may commit an immutable packet that a prior attempt already published, but cannot change or republish it. Generic item-state calls cannot move `ACTIVE`, `RETAINED`, or `PUBLICATION_PENDING` work to allocation `NONE` or `DONE`; `development.terminal_closeout` is retired for new enqueue/claim and remains readable only for historical audit.
