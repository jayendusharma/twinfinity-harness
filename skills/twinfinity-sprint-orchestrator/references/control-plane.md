...
- `role_executor_broker.py`: owner-only v5 readiness preflight, attempt reservation, atomic message claim/campaign attach, kernel isolation policy, canonical receipt pickup/consumption, broker recovery, and terminal attempt/message transaction.
...
For registry-declared v5 endpoints, the trusted readiness broker owns every mechanical control-plane transition. Current v5 launch is deliberately unavailable: after exact readiness validation and before reservation it returns `BROKER_CREDENTIAL_TRANSPORT_NOT_IMPLEMENTED`, because evaluator-visible `auth.json` is not an acceptable boundary. A later exact attempt-bound host Responses proxy may enable launch only with no mounted auth, no environment key, and `requires_openai_auth=false`. The latent kernel start-gate path binds a self-contained reviewed instruction and receipt schema, applies mandatory namespace and per-process limits, and permits one receipt file; its outer transient unit also requires exact attempt-wide memory, task, runtime, and aggregate CPU cgroup limits. Terminal message, campaign, attempt, and broker transitions require positive child-exit or exact inactive-invocation evidence. Immutable SQLite pickup consumption—not caller or filesystem bytes—drives the existing readiness transition; each poison pickup is durably retired as `STALE`, `HOLD`, or `ERROR` without serializing later pickups or ordinary supervisor work. Writer, terminal-watch, and hosted v5 RPCs remain fail-closed, and v4-to-v5 migration rejects both actionable legacy work and the missing proxy prerequisite.
