...
## Claimed-no-delivery PARK boundary

Planner v3 remains a non-coding, read-only endpoint. Its sole narrow mutation bridge is `planner-park/v1`, and that protocol is eligible only for the closed claimed-no-delivery notice schema after the accountable writer has preserved exact dirty diff bytes, completed cleanup, and supplied all bound lease, source-equivalence, graph, capacity, watch, admission, artifact, and repository-observation evidence. Ordinary notices never enter this bridge.

The runner stores only the capability digest, launches the exact pinned Codex/profile/config under a minimal environment with neither raw token nor database path, and owns a private one-use Unix-socket broker. Before prompt release it commits the final heartbeat, requires a `(0,0,0)` TRUNCATE checkpoint and zero WAL, then holds `BEGIN IMMEDIATE` until the exact nested Bash hook and distinct controller process have reauthenticated and adopted the capability. Heartbeats remain paused during that interval. The manifest recursively binds the runner, controller, guard, their repository-local Python imports, the Python and Codex executables, managed requirements, profile, effective config, argv, cwd, cgroups, and process start identities.

The controller consumes before read-only authentication, obtains two byte-identical current registered-Git/GitHub observations outside a write transaction, adopts before opening a writable store, and commits the exact PARK transaction atomically. An already committed matching request takes the official replay path without a second repository observation or release. Any parse, source, process, command, environment, hook, manifest, checkpoint, timeout, replay, adoption, or process-group mismatch is a typed `HOLD`; a denial must leave the target message PREPARED and every controller-owned item, allocation, lease, watch, dirty event, and release surface unchanged. This protocol grants no installation, endpoint-activation, provider, application-delivery, or unrelated SQLite authority.
