...
prepare canonical compact JSON with exactly these fields:
...
state under one `BEGIN IMMEDIATE` transaction. It creates only the canonical
...
canonical receipt digest. Any failure rolls back the complete transaction.
Exact replay validates the pointer and event and returns the byte-identical
receipt without another mutable open or effect.
...
request, digest drift, database replacement, sidecars, or identity drift is a
typed zero-write HOLD. The bootstrap does not activate v2, install source,
