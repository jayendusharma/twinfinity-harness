...
def logical_state(database: Path) -> dict:
...
class PreCanarySchemaBootstrapTests(unittest.TestCase):
    ...
    def test_preview_apply_and_exact_replay_are_closed_and_atomic(self) -> None:
        ...
        before_logical = logical_state(self.database)
        ...
        after_logical = logical_state(self.database)
        for table, rows in before_logical["rows"].items():
            if table not in ("approval_events", "sqlite_sequence"):
                self.assertEqual(rows, after_logical["rows"][table], table)
        self.assertEqual(
            len(before_logical["rows"]["approval_events"]) + 1,
            len(after_logical["rows"]["approval_events"]),
        )
        for row in before_logical["rows"]["approval_events"]:
            self.assertIn(row, after_logical["rows"]["approval_events"])
        for row in before_logical["rows"]["sqlite_sequence"]:
            if row[0] != "approval_events":
                self.assertIn(row, after_logical["rows"]["sqlite_sequence"])
        for item in before_logical["objects"]:
            self.assertIn(item, after_logical["objects"])
        ...
        for failpoint in bootstrap.APPLY_FAILPOINTS:
            with self.subTest(failpoint=failpoint):
                with tempfile.TemporaryDirectory() as name:
                    ...
                    before_logical = logical_state(database)
                    namespace = sorted(
                    ...
                    self.assertEqual(before_logical, logical_state(database))
                    self.assertEqual(
                        namespace,
    ...
    def test_path_replacement_race_is_rejected_by_pinned_identity(self) -> None:
        # External replacement violates sole ownership. This chosen diagnostic
        # interleaving is observed; it is not a promise for arbitrary races.
        preview = self.preview()
    ...
    def test_commit_boundary_anchor_failure_rolls_back_complete_effect(self) -> None:
        ...
        self.assertEqual(4, anchor_calls)
        self.assertEqual(before, self.database.read_bytes())
        self.assertEqual(
            before_namespace,
            sorted(entry.name for entry in self.root.iterdir()),
    ...
    def test_commit_boundary_path_replacement_is_rejected_before_commit(self) -> None:
        # This unsupported interference is deliberately placed before the
        # identity observation. Exclusion after that observation is not claimed.
        preview = self.preview()
        before = self.database.read_bytes()
    ...
    def test_hidden_approval_event_dml_trigger_is_rejected_before_writes(self) -> None:
        def hidden_dml(connection: sqlite3.Connection) -> None:
            connection.execute(
                "CREATE TRIGGER hidden_bootstrap_dml AFTER INSERT ON approval_events "
                "WHEN NEW.event_type='PRE_CANARY_SCHEMA_BOOTSTRAP_APPLIED' "
                "BEGIN INSERT INTO approval_events(event_type,entity_key,"
                "payload_sha256,created_at) VALUES ('HIDDEN_WRITE','hidden',"
                "NEW.payload_sha256,NEW.created_at); END"
            )

        with tempfile.TemporaryDirectory() as name:
            database = build_predecessor(Path(name), hidden_dml)
            request = request_for(database)
            before = database.read_bytes()
            with patch.object(bootstrap, "DEFAULT_DATABASE", database):
                with self.assertRaisesRegex(
                    bootstrap.BootstrapHold,
                    "PRE_CANARY_BOOTSTRAP_PREDECESSOR_OBJECT_DRIFT",
                ):
                    bootstrap.preview_schema_bootstrap(request)
            self.assertEqual(before, database.read_bytes())

    def test_postcommit_connection_cleanup_failure_preserves_applied(self) -> None:
        preview = self.preview()
        ...
        def connect_with_cleanup_failure(database: object, *args: object, **kwargs: object):
            if "?mode=rw" in str(database):
                kwargs["factory"] = CleanupFailureConnection
            return connect(database, *args, **kwargs)

        with patch.object(bootstrap.sqlite3, "connect", side_effect=connect_with_cleanup_failure):
            receipt = self.apply(preview)
        self.assertEqual("APPLIED", receipt["state"])
        self.assertEqual(bootstrap._receipt(self.request, preview), receipt)
        self.assertEqual(receipt, self.apply(preview))

    def test_postcommit_descriptor_cleanup_failure_preserves_applied(self) -> None:
        preview = self.preview()
        close = os.close
        require_anchor = bootstrap._require_anchor
        ...
        class CommitObservedConnection(sqlite3.Connection):
            def execute(self, sql: str, *args: object, **kwargs: object):
                nonlocal committed
                result = super().execute(sql, *args, **kwargs)
                if sql == "COMMIT":
        ...
        def connect_observed(database: object, *args: object, **kwargs: object):
            if "?mode=rw" in str(database):
                kwargs["factory"] = CommitObservedConnection
            return connect(database, *args, **kwargs)

        def observe_anchor(descriptor: int, *args: object, **kwargs: object):
            nonlocal anchor_descriptor
            anchor_descriptor = descriptor
            return require_anchor(descriptor, *args, **kwargs)

        def fail_close(descriptor: int):
            close(descriptor)
            if descriptor == anchor_descriptor and committed:
                raise OSError("synthetic anchor close failure")

        with patch.object(bootstrap.sqlite3, "connect", side_effect=connect_observed), \
                patch.object(bootstrap, "_require_anchor", side_effect=observe_anchor), \
                patch.object(bootstrap.os, "close", side_effect=fail_close):
            receipt = self.apply(preview)
        self.assertTrue(committed)
        self.assertEqual(bootstrap._receipt(self.request, preview), receipt)
        self.assertEqual(receipt, self.apply(preview))

    def test_complete_predecessor_objects_reject_drift_at_every_validation(self) -> None:
        # Canonical approval_events has no explicit trigger/index. Its exact
        # empty family must reject additions. Existing predecessor families
        # exercise missing, renamed, and changed trigger/index definitions.
        cases = {
            "event-trigger": (
                "CREATE TRIGGER hidden_event AFTER INSERT ON approval_events "
                "BEGIN UPDATE coordination_capacity_current SET version=99; END",
            ),
            "event-index": (
                "CREATE INDEX unexpected_event_index ON approval_events(entity_key)",
            ),
            "missing-trigger": ("DROP TRIGGER approval_proposals_immutable_update",),
            "renamed-trigger": (
                "DROP TRIGGER approval_proposals_immutable_update",
                "CREATE TRIGGER renamed_proposal_trigger BEFORE UPDATE ON approval_proposals "
                "BEGIN SELECT RAISE(ABORT, 'APPROVAL_PROPOSAL_IMMUTABLE'); END",
            ),
            "altered-trigger": (
                "DROP TRIGGER approval_proposals_immutable_update",
                "CREATE TRIGGER approval_proposals_immutable_update BEFORE UPDATE ON approval_proposals "
                "BEGIN SELECT 1; END",
            ),
            "missing-index": ("DROP INDEX approval_pending_order",),
            "renamed-index": (
                "DROP INDEX approval_pending_order",
                "CREATE INDEX renamed_pending_order ON approval_proposals(repository,urgency,priority,created_at)",
            ),
            "altered-index": (
                "DROP INDEX approval_pending_order",
                "CREATE INDEX approval_pending_order ON approval_proposals(repository,created_at)",
            ),
            "event-table-check": (
                "DROP TABLE approval_events",
                "CREATE TABLE approval_events (id INTEGER PRIMARY KEY AUTOINCREMENT, "
                "event_type TEXT NOT NULL CHECK(event_type!=''),entity_key TEXT NOT NULL, "
                "payload_sha256 TEXT NOT NULL,created_at TEXT NOT NULL)",
            ),
        }
        for name, statements in cases.items():
            for phase in ("preview", "apply-preflight", "transaction", "replay"):
                with self.subTest(name=name, phase=phase), tempfile.TemporaryDirectory() as root:
                    def mutate(connection: sqlite3.Connection):
                        for statement in statements:
                            connection.execute(statement)

                    database = build_predecessor(
                        Path(root), mutate if phase in ("preview", "apply-preflight") else None
                    )
                    request = request_for(database)
                    preview = bootstrap._preview(request)
                    with patch.object(bootstrap, "DEFAULT_DATABASE", database):
                        if phase == "replay":
                            bootstrap.apply_schema_bootstrap(
                                request, expected_request_sha256=preview["request_sha256"],
                                expected_preview_sha256=preview["preview_sha256"],
                            )
                            connection = sqlite3.connect(database, isolation_level=None)
                            try:
                        ...
                        original_read = bootstrap._read_state
                        reads = 0
                        traced = []

                        def transaction_drift(connection, *args):
                            nonlocal reads
                            reads += 1
                            if phase == "transaction" and reads == 2:
                                mutate(connection)
                                connection.set_trace_callback(traced.append)
                            return original_read(connection, *args)

                        with patch.object(bootstrap, "_read_state", side_effect=transaction_drift):
                            with self.assertRaisesRegex(
                                bootstrap.BootstrapHold,
                                "PRE_CANARY_BOOTSTRAP_PREDECESSOR_OBJECT_DRIFT",
                            ):
                    ...
                    self.assertEqual(before, logical_state(database))
                    if phase == "transaction":
                        self.assertEqual(2, reads)
                        self.assertFalse(any(sql.lstrip().upper().startswith(
                            ("INSERT", "UPDATE", "DELETE", "CREATE")
                        ) for sql in traced), traced)

    def test_prewrite_transaction_identity_drift_has_no_bootstrap_writes(self) -> None:
        preview = self.preview()
        before = logical_state(self.database)
        require_anchor = bootstrap._require_anchor
        read_state = bootstrap._read_state
        traced = []
        calls = 0

        def trace_target(connection, *args):
            connection.set_trace_callback(traced.append)
            return read_state(connection, *args)
        ...
        def fail_prewrite(*args, **kwargs):
            ...
            require_anchor(*args, **kwargs)
            if calls == 3:
                raise bootstrap.BootstrapHold("PRE_CANARY_BOOTSTRAP_DATABASE_IDENTITY_DRIFT")

        with patch.object(bootstrap, "_read_state", side_effect=trace_target), \
                patch.object(bootstrap, "_require_anchor", side_effect=fail_prewrite):
            with self.assertRaisesRegex(bootstrap.BootstrapHold, "DATABASE_IDENTITY_DRIFT"):
                self.apply(preview)
        self.assertEqual(3, calls)
        self.assertFalse(any(sql.lstrip().upper().startswith(
            ("INSERT", "UPDATE", "DELETE", "CREATE")
        ) for sql in traced), traced)
        self.assertEqual(before, logical_state(self.database))

    def test_source_object_digests_bind_event_counter_and_all_predecessor_objects(self) -> None:
        original = bootstrap._canonical_predecessor_objects()
        for table in ("approval_events", "sqlite_sequence", "approval_proposals"):
            with self.subTest(table=table):
                changed = [dict(item) for item in original]
                next(item for item in changed if item["table"] == table)["sql"] += " -- drift"
                with patch.object(bootstrap, "_canonical_predecessor_objects", return_value=changed):
                    with self.assertRaisesRegex(bootstrap.BootstrapHold, "SOURCE_SENTINEL_DRIFT"):
                        bootstrap._require_source_sentinels()

    def test_frozen_preview_rejects_each_substituted_authority_binding(self) -> None:
        preview = self.preview()
        before = logical_state(self.database)
        for field, replacement in {
            "accepted_harness_main_sha": "9" * 40,
            "stopped_state_evidence_sha256": "9" * 64,
            "rollback_evidence_sha256": "9" * 64,
            "v1_authority_sha256": "9" * 64,
            "v1_activated_at": "2026-09-04T21:00:00Z",
            "operation_key": "issue-202-substitution",
        }.items():
            with self.subTest(field=field), patch.object(bootstrap, "DEFAULT_DATABASE", self.database):
                request = {**self.request, field: replacement}
                with self.assertRaisesRegex(bootstrap.BootstrapHold, "REQUEST_DIGEST_DRIFT"):
                    bootstrap.apply_schema_bootstrap(
                        request, expected_request_sha256=preview["request_sha256"],
                        expected_preview_sha256=preview["preview_sha256"],
                    )
